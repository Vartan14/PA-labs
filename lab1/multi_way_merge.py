...
class MultiWayMerge:
    ...
    @staticmethod
    def is_sorted(init_file, file_b1, file_c1):
        return os.path.getsize(init_file) == os.path.getsize(file_b1) or \
               os.path.getsize(init_file) == os.path.getsize(file_c1)

    @staticmethod
    def is_all_empty(input_files: list) -> bool:
        for file in input_files:
            if file.curr:
                return False
        return True

    def first_distribution(self, files_b_paths):
        # open files
    ...
    def merge(self, input_files_paths, output_files_paths):
        input_files = []
        output_files = []
        ...
        seq = []
        # main loop
        while not self.is_all_empty(input_files):
            min_val = MAX_INT
            min_ind = -1
